...
import (
	...
	mockdb "github.com/Country7/backend-captaincode-mysql/db/mock"
...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code) // OK
		...
		},
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusNotFound, recorder.Code) // 404
			},
		},
		{
			name:      "internal error",
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code) // 500
			},
		},
		{
			name:      "invalid ID",
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusBadRequest, recorder.Code) // 400
			},
		},
	}
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().CreateAccount(gomock.Any(), gomock.Eq(db.CreateAccountParams{
					...
					Times(1).
					Return(nil) // Возвращаем только ошибку, которая равна nil

				store.EXPECT().GetLastAccount(gomock.Any()).
					Times(1).
					Return(account, nil) // Возвращаем последний созданный аккаунт и ошибку, которая равна nil
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusOK, recorder.Code) // OK
				requireBodyMatchAccount(t, recorder.Body, account)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().CreateAccount(gomock.Any(), gomock.Eq(db.CreateAccountParams{
					...
					Times(1).
					Return(sql.ErrConnDone) // Возвращаем только ошибку

				store.EXPECT().GetLastAccount(gomock.Any()).
					Times(0) // Ожидаем, что GetLastAccount не будет вызван
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusInternalServerError, recorder.Code) // 500
			},
...
func TestDeleteAccountAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusNoContent, recorder.Code) // 204
			},
		},
		{
			name:      "not found",
